...
import { z } from "zod";
import { updateDoctorSchema } from "../__schema";
...
import { updateDoctorAction } from "../__action";
import {
  Select,
  SelectContent,
  SelectGroup,
...
type TDoctorListUpdateForm = Pick<
  Doctor,
  "id" | "name" | "specialist" | "availableDates"
>;
export function DoctorListUpdateForm(props: TDoctorListUpdateForm) {
  let form = useForm<z.infer<typeof updateDoctorSchema>>({
    defaultValues: {
      id: props.id,
      name: props.name,
  ...
  let onSubmit = form.handleSubmit(async (values) => {
    ...
    let res = await updateDoctorAction(values);
    toast.dismiss();
    ...
    popModal("ModalForm");
    toast.success("Berhasil memperbarui dokter");
  });
